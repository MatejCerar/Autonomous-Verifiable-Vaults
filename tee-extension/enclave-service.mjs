...
// tee-extension/service-entry.ts
...
// tee-extension/extension/vendor/optimize.ts
var DEFAULT_PARAMS = {
  capital: 1e6,
  cap: 0.3,
  maxTotalOut: 0.8,
...
function project(w, capp, T) {
  ...
  for (let it = 0; it < 200; it++) {
    const tau2 = 0.5 * (lo + hi);
    let s = 0;
    for (let i = 0; i < n; i++) s += Math.min(capp[i], Math.max(0, x[i] - tau2));
    if (s > T) lo = tau2;
    else hi = tau2;
  }
  const tau = hi;
...
function scanMax1d(f, lo, hi, samples) {
...
function refine(wIn, venues, capp, Sigma, C, T, p) {
  const n = wIn.length;
  const w = [...wIn];
  const clamp = (x, i) => Math.min(capp[i], Math.max(0, x));
...
function normalizeMarketData(md, pIn, C) {
  const assumptions = [];
  let p = { ...pIn };
  let assets;
  ...
  const venues = assets.map((a, idx) => {
    ...
    if (vol < p.depegVol) vol = p.depegVol;
    let cap = p.cap;
...
function optimize(marketData, params) {
  ...
  for (let k = 0; k < pEff.iterations; k++) {
    const step = pEff.step0 / (1 + 5e-4 * k);
    const Sw2 = covTimes(Sigma, w);
    const cand = w.map(
      (wi, i) => wi + step * (marginalYield(wi, venues[i], C, pEff) - pEff.lambda * Sw2[i])
    );
  ...
  const expectedApy = venues.reduce((s, v, i) => s + yieldContribution(w[i], v, C, pEff), 0) + reserve * pEff.reserveApy;
...
function buildRationale(perVenue, reserve, p, T, assumptions) {
  const clamped = perVenue.filter((v) => v.bindingConstraint === "venue cap" || v.bindingConstraint === "liquidity").map((v) => v.symbol);
  const interior = perVenue.filter((v) => v.bindingConstraint.startsWith("interior")).map((v) => v.symbol);
  const excluded = perVenue.filter((v) => v.bindingConstraint.startsWith("excluded")).map((v) => v.symbol);
  const parts = [];
  parts.push(
...
// tee-extension/extension/avv.ts
var PLAN_ABI = [
  {
    type: "tuple",
    components: [
...
function weightToAmount(weight, capital1e18) {
  if (!Number.isFinite(weight) || weight <= 0) return 0n;
  const micro = BigInt(Math.round(weight * 1e12));
  return capital1e18 * micro / 1000000000000n;
}
function buildPlan(marketData, params = {}) {
  const venueCap = params.venueCap ?? DEFAULT_PARAMS.cap;
  const reserveFloor = params.reserveFloor ?? DEFAULT_PARAMS.reserveFloor;
  const result = optimize(marketData, { cap: venueCap, reserveFloor });
  const capital1e18 = 1000000n * ONE;
  const capVenueAmount = weightToAmount(venueCap, capital1e18);
  const reserveFloorAmount = weightToAmount(reserveFloor, capital1e18);
  const byVenueId = /* @__PURE__ */ new Map();
  for (const pv of result.perVenue) {
    ...
    if (amount > capVenueAmount) amount = capVenueAmount;
    byVenueId.set(venueId, (byVenueId.get(venueId) ?? 0n) + amount);
  }
  const allocations = [0n, 1n, 2n].map((venueId) => ({
    venueId,
  ...
  const now = params.nowSecs ?? BigInt(Math.floor(Date.now() / 1e3));
...
function computePlanHash(controller, chainId, plan) {
...
async function signPlan(plan, controller, chainId, teeSignerKey) {
...
var PLAN_WITH_SIG_ABI = [PLAN_ABI[0], { name: "signature", type: "bytes" }];
function encodePlanWithSig(plan, signature) {
  return encodeAbiParameters(PLAN_WITH_SIG_ABI, [plan, signature]);
}

// tee-extension/extension/vendor/marketData.ts
var VENDORED_MARKET_DATA = {
  "asOf": "2026-08-13T07:24:00Z",
  "chainId": 14,
  "chainName": "Flare mainnet",
  "protocol": "Mystic Finance (Morpho Vault V2 over standalone Morpho Blue)",
  "app": "https://app.mysticfinance.xyz",
  "note": "Mystic 'Core' vaults are Morpho Vault V2 ERC-4626 vaults (adapter-based, NOT MetaMorpho V1 queues). Each vault routes into 3 underlying Morpho Blue markets via a single liquidity adapter. Morpho's hosted API excludes chainId 14, so market state is read directly from Morpho Blue and rates are recomputed from AdaptiveCurveIrm; supply APY headline is cross-checked against DefiLlama yields API.",
  "contracts": {
  ...
  "prices": {
    "FLR_USD": {
      "value": 59951e-7,
      "source": "FTSO V2 getFeedById(FLR/USD) live @ block ts 2026-08-13T07:19:35Z; cross-checked CoinMarketCap $0.006071 / Coinbase $0.005975",
      "confidence": "high"
    },
    "XRP_USD": {
      "value": 1.013054,
      "source": "FTSO V2 getFeedById(XRP/USD) live @ 2026-08-13T07:19:35Z",
      "confidence": "high"
    },
    "USDT0_USD": {
      "value": 1,
      "source": "ASSUMED PEG (stablecoin, USDT0 = LayerZero USDT). Treat as 1.0 with small depeg tail.",
  ...
  "assets": [
    {
      ...
      "vaultSymbol": "CSXRP",
      "vaultDecimals": 18,
      "vaultType": "Morpho Vault V2",
      "sharePriceAssetsPerShare": 1.000432,
      "totalAssetsTokens": 21813705582e-4,
      "totalSupplyShares": 2180427438063e-6,
      "supplyApy": 366e-5,
      "apyBase": 366e-5,
      "apyReward": 0,
      "supplyApyOnchainComputed": 677e-5,
      "utilization": 0.8302,
      "availableLiquidityUsd": 12e5,
      "availableLiquidityNote": "~$198k instantly withdrawable from adapter (market idle) PLUS ~$1.0M idle FXRP held in the vault buffer (totalAssets 2.18M FXRP vs 1.15M supplied to Morpho). So instant vault liquidity ~ $1.2M; source: on-chain.",
      "tvlUsd": 2201182,
      "lltv": 0.625,
      "lltvNote": "Dominant underlying market (93% weight) has LLTV 62.5% (collateral USDT0); other two markets 77%.",
      "originationFee": 0,
      "performanceFee": 0.1,
      "priceUsd": 1.013054,
      "annualizedVol": 0.55,
      "annualizedVolSource": "ESTIMATE. XRP 30-90d realized vol typically 0.50-0.65 annualized; XRP/USD $1.013 live. Labeled estimate.",
      "sources": {
        "supplyApy": "DefiLlama yields pool ae77c3b6-f9ca-49a8-b24d-08c12ac2d7b4 (apy=0.36551%)",
        "supplyApyOnchainComputed": "Recomputed from Morpho Blue market state + AdaptiveCurveIrm rateAtTarget; vault-weighted, net 10% perf fee",
        "utilization": "on-chain: sum(totalBorrowAssets)/sum(totalSupplyAssets) weighted by adapter supply across 3 markets",
        "tvlUsd": "DefiLlama tvlUsd",
        "lltv": "Morpho Blue idToMarketParams",
        "priceUsd": "FTSO V2 XRP/USD proxy"
      }
    },
    {
      "symbol": "USDT0",
      ...
      "vaultSymbol": "COREUSDT0",
      "vaultDecimals": 18,
      "vaultType": "Morpho Vault V2",
      "sharePriceAssetsPerShare": 1.01843,
      "totalAssetsTokens": 21094814832348e-6,
      "totalSupplyShares": 20713042121902e-6,
      "supplyApy": 0.03278,
      "apyBase": 0.03278,
      "apyReward": 0,
      "supplyApyOnchainComputed": 0.0337,
      "utilization": 0.8489,
      "availableLiquidityUsd": 3187888,
      "availableLiquidityNote": "~$3.19M instantly withdrawable (market idle across 3 markets); on-chain. Vault holds negligible idle buffer.",
      "tvlUsd": 21074299,
      "lltv": 0.77,
      "lltvNote": "Dominant market (98.5% weight) LLTV 77% (collateral FXRP); other two markets 62.5%.",
      "originationFee": 0,
      "performanceFee": 0.1,
      "priceUsd": 1,
      "annualizedVol": 0.01,
      "annualizedVolSource": "ESTIMATE. USDT0 = LayerZero-bridged USDT, treated as stable; small depeg tail. 1% annualized proxy vol for the risk model.",
      "sources": {
        "supplyApy": "DefiLlama yields pool 5ea185f4-b3cc-4fd8-afe6-39037bf483dd (apy=3.27832%)",
        "supplyApyOnchainComputed": "Recomputed on-chain, matches DefiLlama to ~0.1pp",
        "utilization": "on-chain weighted",
        "tvlUsd": "DefiLlama",
        "lltv": "Morpho Blue idToMarketParams"
      }
    },
    {
      ...
      "vaultName": "Core wFLR",
      "vaultSymbol": "COREWFLR",
      "vaultDecimals": 18,
      "vaultType": "Morpho Vault V2",
      "sharePriceAssetsPerShare": 1.002845,
      "totalAssetsTokens": 93175628728072e-6,
      "totalSupplyShares": 92911297439615e-6,
      "supplyApy": 0.04589,
      "apyBase": 0.04589,
      "apyReward": 0,
      "supplyApyOnchainComputed": 0.04711,
      "utilization": 0.9431,
      "availableLiquidityUsd": 31759,
      "availableLiquidityNote": "ONLY ~$32k (5.3M WFLR) instantly withdrawable: markets run very hot (92-98.6% util). Large redemptions would need forceDeallocate or wait. On-chain.",
      "tvlUsd": 560794,
      "lltv": 0.86,
      "lltvNote": "Largest market (67% weight) LLTV 86% (collateral rFLR/staked-FLR type); other two 62.5%.",
      "originationFee": 0,
      "performanceFee": 0.1,
      "priceUsd": 59951e-7,
      "annualizedVol": 0.8,
      "annualizedVolSource": "ESTIMATE. FLR at all-time-low ($0.00588 on 2026-08-05), elevated recent vol; 30-90d realized est 0.70-0.90 annualized. Labeled estimate.",
      "sources": {
        "supplyApy": "DefiLlama yields pool cd22e73a-e73d-48b1-a919-2522b109119a (apy=4.58941%)",
        "supplyApyOnchainComputed": "Recomputed on-chain, matches DefiLlama to ~0.1pp",
        "utilization": "on-chain weighted",
        "tvlUsd": "DefiLlama",
        "lltv": "Morpho Blue idToMarketParams",
        "priceUsd": "FTSO V2 FLR/USD"
      }
    }
  ],
  "irm": {
    "type": "AdaptiveCurveIrm",
    "address": "0xE5B5627C5973AfAE1928a6b8e5c1D6AABFEC8a7a",
    "targetUtilization": 0.9,
    "curveSteepness": 4,
    "shape": "Piecewise-linear kink at 90% util. Below target, borrow rate scales from rateAtTarget/4 (u=0) up to rateAtTarget (u=90%). Above target it scales up to 4x rateAtTarget at u=100%, and rateAtTarget itself drifts up over time if util stays high (adaptive). Supply APY = borrowAPY * utilization * (1 - marketFee=0). Marginal supply yield to a NEW depositor DROPS as deposit lowers utilization: a deposit of size d into a market with supply S, borrow B moves util from B/S to B/(S+d), so marginal yield falls roughly proportional to util decrease, steeply if you were above 90%.",
    "rateAtTargetPerSecWad": {
      "FXRP_dominant": 291842396,
      "USDT0_dominant": 1409185676,
      "WFLR_dominant": 1148093065
    }
  },
  "pairwiseCorrelation": {
    "order": [
    ...
    "source": "ESTIMATE for the risk model. FXRP price-driver = XRP, WFLR = FLR. XRP-FLR correlation ~0.4-0.5 (both crypto-beta, FLR partly an XRP-ecosystem asset). USDT0 stable => ~0 correlation to both. These are yield-relevant price-risk correlations, not APY correlations.",
    "confidence": "estimate"
  },
  "confidenceSummary": {
    "live_onchain": [
...
// src/live.ts
var FLARE_RPC = "https://flare-api.flare.network/ext/C/rpc";
var CHAIN_ID = 14;
var FLR_USD_ID = "0x01464c522f55534400000000000000000000000000";
var XRP_USD_ID = "0x015852502f55534400000000000000000000000000";
var flareChain = {
  id: CHAIN_ID,
  name: "Flare",
...
async function readFeed(client, ftso, feedId) {
...
async function resolveFtso(client) {
  try {
    const addr = await client.readContract({
      address: addresses.contractRegistry,
  ...
  return { address: addresses.ftsoV2, source: "fallback" };
}
...
function matchPool(pools, token) {
  ...
  const hit = mystic.find(
    (p) => (p.underlyingTokens ?? []).some((u) => u.toLowerCase() === t)
  );
  return hit ?? null;
}
...
function snapAsset(snap, symbol) {
  return snap?.assets.find((a) => a.symbol === symbol);
}
async function assembleMarketData(client) {
  ...
  const assets = addresses.venues.map((v) => {
    ...
    let tvlUsd;
    const onchainTvl = vaultTotals[v.symbol] != null ? Number(formatUnits(vaultTotals[v.symbol], v.decimals)) * priceUsd : void 0;
  ...
  const market = {
    ...
    app: addresses.app,
    note: "LIVE cycle: FTSO prices + vault totalAssets read on-chain this click; supply APY + TVL from DefiLlama; utilization, vol, correlation and the USDT0 peg are labeled snapshot estimates for the risk model.",
    prices: {
      FLR_USD: {
        value: flr.price,
...
function toOptimizerInput(market) {
  return {
    asOf: market.asOf,
...
async function readMarketForEnclave() {
...
var SIGNER = process.env.TEE_PLAN_SIGNER_KEY || "0x6fccfd706182c24a8c22f959077d79fd937673a2625dc77c880422f50c77c38a";
var CONTROLLER = process.env.AVV_CONTROLLER || "0x47A4E99f26F3ABF95Ca09f81399ECf84b04F3909";
var CHAIN_ID2 = BigInt(process.env.AVV_CHAIN_ID || "114");
var CODE_HASH = process.env.AVV_CODE_HASH || "0x4c1ea64a6a81ce2cae7c1aaff36fcdbc64d9b06608471a1d346a9de1e31763ea";
var MODEL_VERSION = BigInt(process.env.AVV_MODEL_VERSION || "1");
var server = http2.createServer(async (req, res) => {
  ...
  if (req.method !== "POST" || !req.url?.startsWith("/curation")) {
    res.writeHead(404);
    res.end("GET /market or POST /curation");
    return;
  }
  let body = "";
  ...
  try {
    ...
    try {
      ...
      market = j.market ?? VENDORED_MARKET_DATA;
    }
    const plan = buildPlan(market, { modelVersion: MODEL_VERSION, codeHash: CODE_HASH, nonce });
    const signature = await signPlan(plan, CONTROLLER, CHAIN_ID2, SIGNER);
