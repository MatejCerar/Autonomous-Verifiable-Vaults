...
contract HelloWorldInstructionSender {
    ...
    /// @notice Operation type for AVV curation actions.
    // forge-lint: disable-next-line(unsafe-typecast)
    bytes32 public constant OP_TYPE_CURATION = bytes32("CURATION");

    /// @notice Command for the AVV allocation cycle.
    // forge-lint: disable-next-line(unsafe-typecast)
    bytes32 public constant OP_COMMAND_CYCLE = bytes32("CYCLE");
    ...
    /// @notice Sends an AVV CURATION/CYCLE instruction to the TEE.
    /// @param _message Hex-encoded UTF-8 JSON market snapshot, or empty to let the
    ///                 enclave use its bundled snapshot.
    function sendCuration(bytes calldata _message) external payable {
        address[] memory teeIds = TEE_MACHINE_REGISTRY.getRandomTeeIds(_getExtensionId(), 1);
        address[] memory cosigners = new address[](0);
