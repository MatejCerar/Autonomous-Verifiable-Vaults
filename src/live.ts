...
// --------------------------------------------------------------------------
// Public: lightweight LIVE price read (FTSO only, no DefiLlama)
// --------------------------------------------------------------------------
// The full runCycle pulls a large DefiLlama payload which can be slow/flaky in
// the browser. For the Live Inputs card we only need the two on-chain FTSO
// prices, so read just those (fast, robust) and refresh them on a timer.
export interface LivePrices {
  flrUsd: number;
  xrpUsd: number;
  at: string;
  ftsoSource: "registry" | "fallback";
}

// Read prices from COSTON2 (the same RPC host the rest of the app uses) rather
// than Flare mainnet, so the Live Inputs work even where the mainnet RPC host is
// unreachable. FLR/USD and XRP/USD are the same global feeds on Coston2 FtsoV2.
const COSTON2_PRICE_RPC = "https://coston2-api.flare.network/ext/C/rpc";
const coston2PriceChain = {
  id: 114,
  name: "Coston2",
  nativeCurrency: { name: "Coston2 Flare", symbol: "C2FLR", decimals: 18 },
  rpcUrls: { default: { http: [COSTON2_PRICE_RPC] } },
} as const;

// Coston2 FtsoV2 (block-latency feeds), resolved via ContractRegistry. Hardcoded
// so we do not depend on a registry read that could return the wrong (mainnet)
// fallback address in some browsers.
const COSTON2_FTSOV2 = "0xC4e9c78EA53db782E28f28Fdf80BaF59336B304d" as Hex;

export async function readLivePrices(): Promise<LivePrices> {
  const client = createPublicClient({
    chain: coston2PriceChain,
    transport: http(COSTON2_PRICE_RPC),
  }) as PublicClient;
  const [flr, xrp] = await Promise.all([
    readFeed(client, COSTON2_FTSOV2, FLR_USD_ID),
    readFeed(client, COSTON2_FTSOV2, XRP_USD_ID),
  ]);
  return {
    flrUsd: flr.price,
    xrpUsd: xrp.price,
...
// --------------------------------------------------------------------------
// Public: build the on-chain Plan for a cycle WITHOUT any network
// --------------------------------------------------------------------------
// The button writes this plan to the AllocationDisplay vault when the real TEE
// proxy is offline (local fallback). Same optimizer, same USD-1e18 amounts the
// enclave emits; only the enclave signature is skipped. Pure and synchronous so
// the button always has a plan even with no chain reachable.
...
// Same code hash the enclave stamps (see signPlan / avv.ts).
const PLAN_CODE_HASH =
  "0x4c1ea64a6a81ce2cae7c1aaff36fcdbc64d9b06608471a1d346a9de1e31763ea" as Hex;

/** Run the optimizer on a market snapshot and build the on-chain Plan. Pure. */
export function planFromMarketData(
  market: MarketData,
  capital = 1_000_000,
): LocalPlan {
  const params: CycleParams = {
    capital,
    receiver: ZERO,
    cap: DEFAULT_CYCLE_PARAMS.cap,
    maxTotalOut: DEFAULT_CYCLE_PARAMS.maxTotalOut,
    reserveFloor: DEFAULT_CYCLE_PARAMS.reserveFloor,
  };
  const { app: opt } = runOptimizer(market, params);
  const allocations = opt.perVenue.map((v) => ({
    venueId: BigInt(v.venueId),
    amount: usdToWad(v.amountUsd),
  }));
  const totalOut = usdToWad(capital);
  const reserveAmount =
    totalOut - allocations.reduce((a, x) => a + x.amount, 0n);
  const planId = keccak256(
    toHex(JSON.stringify({ asOf: opt.asOf, weights: opt.weights, capital })),
  );
  return {
    planId,
    modelVersion: 1n,
    codeHash: PLAN_CODE_HASH,
    nonce: 0n,
...
/**
 * Build the on-chain Plan from a completed live cycle. Prefers the cycle's
 * signed envelope (already USD-1e18); falls back to recomputing from the
 * cycle's market snapshot.
 */
export function planFromCycle(cycle: CycleResult, capital = 1_000_000): LocalPlan {
  const env = cycle.envelope;
  if (env) {
    return {
      planId: env.plan.planId as Hex,
      modelVersion: BigInt(env.plan.modelVersion),
      codeHash: env.plan.codeHash as Hex,
      nonce: BigInt(env.plan.nonce),
      expiry: BigInt(env.plan.expiry),
      totalOut: BigInt(env.plan.totalOut),
      reserveAmount: BigInt(env.plan.reserveAmount),
      allocations: env.plan.allocations.map((a) => ({
        venueId: BigInt(a.venueId),
        amount: BigInt(a.amount),
      })),
    };
